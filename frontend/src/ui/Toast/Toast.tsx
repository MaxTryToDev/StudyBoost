...
export function Toast() {
  ...
  React.useEffect(() => {
    ...
    let [x, y] = currentToast.position.split("-") as any;
  ...
  React.useEffect(() => {
    ...
    const timeoutId = setTimeout(
      ...
      (currentToast.duration || 1) * (toastId + 1),
    );
        ...
        <section>
          <ol>
            {toast.map((t: ToastProps, index: number) => {
              return (
                <li
                  className={`toast flex flex-row items-center border-solid border-[1px] border-gray-200 justify-center fixed p-3 shadow-lg rounded-xl max-w-[26.25rem] ${style[t.style ? t.style : "info"].container}`}
                  ref={currentToastRef}
                  data-id={index}
                  ...
                  <div
                    ...
                    <div className={"flex flex-col"}>
                      <h4
                        className={`text-sm font-semibold text-gray-800 w-full ${style[t.style ? t.style : "info"].title}`}>
                        {t.title}
                      </h4>
